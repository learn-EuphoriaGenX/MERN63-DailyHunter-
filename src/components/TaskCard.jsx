...
const TaskCard = ({ task, retriveAllTasks }) => {
  let handleDelete = async (id) => {
    try {
      let token = localStorage.getItem('token')

      let response = await axios.delete(`${BACKENDURL}/api/task/delete/${id}`, {
        headers: {
          Authorization: token,
        },
      })
    ...
    >
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex space-x-2">
          ...
          <button onClick={() => handleDelete(task._id)} className="text-gray-400 hover:text-red-400 transition-colors p-1">
            <svg
              className="w-5 h-5"
              fill="none"
              stroke="currentColor"
